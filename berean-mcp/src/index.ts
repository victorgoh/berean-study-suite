...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // --- Authentication & Authorization Gatekeeper ---
    // If API_KEY is configured in Cloudflare environment secrets, enforce validation
    ...
    if (url.pathname === "/tools/bible_lookup" && request.method === "POST") {
      ...
      const res = await lookupBiblePassage(env, body.version || "BSB", body.reference || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/bible_search" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await searchBible(env, body.query || "", body.version || "BSB", body.book_filter, body.limit || 50);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/commentary_lookup" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupCommentary(env, body.version || "Henry", body.reference || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/cross_references" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupCrossReferences(env, body.reference || "", body.limit || 15);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/lexicon_lookup" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupLexiconEntry(env, body.strongs_number || "", body.lexicon || "strongs");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/morphology_lookup" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupMorphology(env, body.reference || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/topic_study" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupTopic(env, body.query || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/character_lookup" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupCharacter(env, body.name || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/location_lookup" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupLocation(env, body.location || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
    ...
    if (url.pathname === "/tools/theological_dictionary" && request.method === "POST") {
      ...
      const res = body.source === "isbe"
        ? await lookupEncyclopedia(env, body.term || "", "isbe")
        : await lookupDictionary(env, body.term || "", body.source || "easton");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/parallel_passages" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupParallels(env, body.query || "", body.include_text ?? true, body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/biblical_promises" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupPromises(env, body.topic || "", body.include_text ?? true, body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/book_analysis" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupBookAnalysis(env, body.book || "", body.section);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/chapter_summary" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupChapterSummary(env, body.book || "", body.chapter || 1);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/bible_names" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupBibleNames(env, body.query || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/chronology" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await lookupChronology(env, body.query || "");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
    ...
    if (url.pathname === "/tools/sermon_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getSermonStudyPack(env, body.reference || "Romans 8:1-4", body.version || "BSB", body.include_xrefs ?? true);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/devotional_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getDevotionalStudyPack(env, body.reference || "Psalm 23", body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/passage_exegesis_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getPassageExegesisPack(env, body.reference || "John 1:1-5", body.version || "BSB", body.include_original ?? true);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/word_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getWordStudyPack(env, body.strongs_number || "G2842", body.reference, body.lexicon || "strongs");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/topic_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getTopicStudyPack(env, body.topic || "Justification", body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/commentary_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getCommentaryStudyPack(env, body.reference || "Romans 8:28", body.commentators || ["Henry", "JFB", "Calvin", "MacL", "Barnes", "Spur"]);
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/lesson_creator_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getLessonCreatorStudyPack(env, body.reference || "Luke 15:11-32", body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/prayer_guide_study_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getPrayerGuideStudyPack(env, body.reference || "Psalm 23", body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }

    if (url.pathname === "/tools/covenant_theology_pack" && request.method === "POST") {
      const body = (await request.json().catch(() => ({}))) as any;
      const res = await getCovenantTheologyPack(env, body.reference || "Genesis 15", body.version || "BSB");
      return new Response(JSON.stringify(res), { headers: { "Content-Type": "application/json", ...corsHeaders } });
    }
