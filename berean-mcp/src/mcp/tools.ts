...
export const CommentaryLookupSchema = {
  version: z.string().default("Henry").describe("Commentary version or author: 'TNotes' (Tyndale Open Study Notes - CC BY-SA 4.0), 'Henry' (Matthew Henry), 'JFB' (Jamieson-Fausset-Brown), 'Calvin' (John Calvin), 'Barnes' (Albert Barnes), 'MacL' (Alexander Maclaren), 'HH' (Charles Simeon), 'Gill' (John Gill), 'Spur' (Charles Spurgeon), 'KD' (Keil & Delitzsch), 'EGNT' (Expositor's Greek NT), 'CECNT' (H.A.W. Meyer), 'BI' (Biblical Illustrator), 'ECER' (Charles Ellicott), 'EBC' (Expositor's Bible), 'Rob' (A.T. Robertson), 'Vincent' (Marvin Vincent), 'Wesley' (John Wesley), 'Clarke' (Adam Clarke), 'Benson' (Joseph Benson), 'Whedon' (Daniel Whedon), 'Lange' (John Peter Lange)"),
